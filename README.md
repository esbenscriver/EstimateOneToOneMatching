...
where $\log p^{X}_{xy}(\theta)$, $ p^{Y}_{xy}(\theta)$, $ p^{X}_{x0}(\theta)$, and $ p^{Y}_{0y}(\theta)$ are the choice probabilities of agents of type X and Y consistent with $t^{*}_{xy}(\theta)$.